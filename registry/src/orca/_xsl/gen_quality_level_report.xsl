...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="ro:collection">
       ...
       <xsl:choose>
	       	<xsl:when test="not(ro:description[@type='rights']) and not(ro:description[@type='accessRights']) and not(ro:rights)">
				<span class="qa_error" level="3" field_id="errors_description" qa_id="REQ_RIGHT">At least one description of the rights, licences or access rights relating to the Collection is required</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="3" field_id="errors_description" qa_id="REQ_RIGHT">At least one description of the rights, licences or access rights relating to the Collection is required</span>
			</xsl:otherwise>
	   </xsl:choose>
        
        <xsl:choose>
	       	<xsl:when test="not(ro:location/ro:address)">
     			<span class="qa_error" level="3" field_id="errors_location" qa_id="REQ_LOCATION_ADDRESS">At least one location address is required for the Collection.</span>
			</xsl:when>
			<xsl:otherwise>
     			<span class="qa_ok" level="3" field_id="errors_location" qa_id="REQ_LOCATION_ADDRESS">At least one location address is required for the Collection.</span>
			</xsl:otherwise>
	   </xsl:choose>
        ...
        <xsl:choose>
	       	<xsl:when test="not(ro:subject) or not(ro:subject[string-length(.) &gt; 0] and ro:subject[string-length(@type) &gt; 0])">
				<span class="qa_error" level="4" field_id="errors_subject" qa_id="REC_SUBJECT">At least one subject (e.g. anzsrc-for code) is recommended for the Collection.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_subject" qa_id="REC_SUBJECT">At least one subject (e.g. anzsrc-for code) is recommended for the Collection.</span>
			</xsl:otherwise>
        </xsl:choose>
        
        <xsl:choose>
	       	<xsl:when test="not(ro:coverage/ro:spatial)">
				<span class="qa_error" level="4" field_id="errors_coverage" qa_id="REC_SPATIAL_COVERAGE">At least one spatial coverage for the Collection is recommended.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_coverage" qa_id="REC_SPATIAL_COVERAGE">At least one spatial coverage for the Collection is recommended.</span>
			</xsl:otherwise>
	    </xsl:choose>
        ...
        <xsl:choose>
	       	<xsl:when test="not(ro:coverage/ro:temporal/ro:date[@type='dateFrom']) and not(ro:coverage/ro:temporal/ro:date[@type = 'dateTo'])">
				<span class="qa_error" level="4" field_id="errors_coverage" qa_id="REC_TEMPORAL_COVERAGE">At least one temporal coverage entry for the collection is recommended.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_coverage" qa_id="REC_TEMPORAL_COVERAGE">At least one temporal coverage entry for the collection is recommended.</span>
			</xsl:otherwise>
	   </xsl:choose>
    ...
    <xsl:template match="ro:party">
	    ...
	    <xsl:choose>
	       	<xsl:when test="not(contains($relatedObjectClassesStr, 'Collection') or ro:relatedObject/ro:key[@roclass = 'Collection'] or ro:relatedObject/ro:key[@roclass = 'collection'])">
				<span class="qa_error" level="3" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_COLLECTION">The Party must be related to at least one Collection record.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="3" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_COLLECTION">The Party must be related to at least one Collection record.</span>
			</xsl:otherwise>
	   </xsl:choose>
	          
        <xsl:choose>
	       	<xsl:when test="not(ro:identifier)">
				<span class="qa_error" level="4" field_id="errors_identifier" qa_id="REC_IDENTIFIER">At least one identifier is recommended for the Party.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_identifier" qa_id="REC_IDENTIFIER">At least one identifier is recommended for the Party.</span>
			</xsl:otherwise>
        </xsl:choose>
        
        <xsl:choose>
	       	<xsl:when test="not(ro:location/ro:address)">
     			<span class="qa_error" level="4" field_id="errors_location" qa_id="REQ_LOCATION_ADDRESS">At least one location address is recommended for the Party.</span>
			</xsl:when>
			<xsl:otherwise>
     			<span class="qa_ok" level="4" field_id="errors_location" qa_id="REQ_LOCATION_ADDRESS">At least one location address is recommended for the Party.</span>
			</xsl:otherwise>
	   </xsl:choose>
         
       <xsl:choose>
	       	<xsl:when test="not(contains($relatedObjectClassesStr, 'Activity') or ro:relatedObject/ro:key[@roclass = 'Activity'] or ro:relatedObject/ro:key[@roclass = 'activity'])">
				<span class="qa_error" level="4" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_ACTIVITY">It is recommended that the Party be related to at least one Activity record.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_ACTIVITY">It is recommended that the Party be related to at least one Activity record.</span>
			</xsl:otherwise>
	   </xsl:choose>
        ...
        <xsl:choose>
	       	<xsl:when test="not(ro:subject) or not(ro:subject[string-length(.) &gt; 0] and ro:subject[string-length(@type) &gt; 0])">
				<span class="qa_error" level="4" field_id="errors_subject" qa_id="REC_SUBJECT">At least one subject (e.g. anzsrc-for code) is recommended for the Party.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_subject" qa_id="REC_SUBJECT">At least one subject (e.g. anzsrc-for code) is recommended for the Party.</span>
			</xsl:otherwise>
        </xsl:choose>

        <xsl:choose>
	       	<xsl:when test="not(ro:existenceDates)">
				<span class="qa_error" level="4" field_id="errors_existenceDates" qa_id="REC_EXISTENCEDATE">Existence dates are recommended for the Party.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_existenceDates" qa_id="REC_EXISTENCEDATE">Existence dates are recommended for the Party.</span>
			</xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="ro:activity">
        ...
        <xsl:choose>
	       	<xsl:when test="not(contains($relatedObjectClassesStr, 'Party') or ro:relatedObject/ro:key[@roclass = 'Party'] or ro:relatedObject/ro:key[@roclass = 'party'])">
				<span class="qa_error" level="3" field_id="errors_relatedObject" qa_id="REQ_RELATED_OBJECT_PARTY">The Activity must be related to at least one Party record.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="3" field_id="errors_relatedObject" qa_id="REQ_RELATED_OBJECT_PARTY">The Activity must be related to at least one Party record.</span>
			</xsl:otherwise>
	   </xsl:choose>
	   
	   <xsl:choose>
	       	<xsl:when test="not(ro:location/ro:address)">
     			<span class="qa_error" level="4" field_id="errors_location" qa_id="REQ_LOCATION_ADDRESS">At least one location address is recommended for the Activity.</span>
			</xsl:when>
			<xsl:otherwise>
     			<span class="qa_ok" level="4" field_id="errors_location" qa_id="REQ_LOCATION_ADDRESS">At least one location address is recommended for the Activity.</span>
			</xsl:otherwise>
	   </xsl:choose>
	   
	    <xsl:choose>
	       	<xsl:when test="not(contains($relatedObjectClassesStr, 'Collection') or ro:relatedObject/ro:key[@roclass = 'Collection'] or ro:relatedObject/ro:key[@roclass = 'collection'])">
				<span class="qa_error" level="4" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_COLLECTION">The Activity must be related to at least one Collection record if available.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_COLLECTION">The Activity must be related to at least one Collection record if available.</span>
			</xsl:otherwise>
	   </xsl:choose>
	   
	   <xsl:choose>
	       	<xsl:when test="not(ro:subject) or not(ro:subject[string-length(.) &gt; 0] and ro:subject[string-length(@type) &gt; 0])">
				<span class="qa_error" level="4" field_id="errors_subject" qa_id="REC_SUBJECT">At least one subject (e.g. anzsrc-for code) is recommended for the Activity.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_subject" qa_id="REC_SUBJECT">At least one subject (e.g. anzsrc-for code) is recommended for the Activity.</span>
			</xsl:otherwise>
        </xsl:choose>

        <xsl:choose>
	       	<xsl:when test="not(ro:existenceDates)">
				<span class="qa_error" level="4" field_id="errors_existenceDates" qa_id="REC_EXISTENCEDATE">Existence dates are recommended for the Activity.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_existenceDates" qa_id="REC_EXISTENCEDATE">Existence dates are recommended for the Activity.</span>
			</xsl:otherwise>
        </xsl:choose>
	</xsl:template>
    ...
    <xsl:template match="ro:service">
	    ...
	    <xsl:choose>
	       	<xsl:when test="not(contains($relatedObjectClassesStr, 'Collection') or ro:relatedObject/ro:key[@roclass = 'Collection'] or ro:relatedObject/ro:key[@roclass = 'collection'])">
				<span class="qa_error" level="3" field_id="errors_relatedObject" qa_id="REQ_RELATED_OBJECT_COLLECTION">The Service must be related to at least one Collection record.</span>
			</xsl:when>
			<xsl:otherwise>
       ...
       <xsl:choose>
			...
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_relatedObject" qa_id="REC_RELATED_OBJECT_PARTY">It is recommended that the Service be related to at least one Party record.</span>
			</xsl:otherwise>
	   </xsl:choose> 
	    ...
	    <xsl:choose>
	        <xsl:when test="not(ro:accessPolicy)">
				<span class="qa_error" level="4" field_id="errors_description" qa_id="REQ_ACCESS_POLICY">At least one Access Policy URL is recommended for the Service record.</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="qa_ok" level="4" field_id="errors_description" qa_id="REQ_ACCESS_POLICY">At least one Access Policy URL is recommended for the Service record.</span>
			</xsl:otherwise>
	    </xsl:choose>
