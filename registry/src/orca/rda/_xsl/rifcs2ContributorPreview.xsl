...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="ro:party">
		...
		<div id="left">           
                ...
                <h1>    <xsl:apply-templates select="ro:displayTitle"/> </h1>
                </xsl:when>
                <xsl:when test="ro:name[@type = 'primary']"><h1>
                        <xsl:apply-templates select="ro:name[@type = 'primary']"/>
                        <xsl:apply-templates select="ro:name[@type = 'alternative']"/></h1>
                </xsl:when>
                <xsl:when test="ro:name"><h1>
