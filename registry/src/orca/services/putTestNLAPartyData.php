...
define('gNLA_TEST_SRU_URI','http://www-test.nla.gov.au/apps/srw/search/peopleaustralia');
...
if($partyIdentifiers)
{
	...
	//	$requestURI =  gNLA_SRU_URI."?query=rec.identifier=%22".$partyId."%22&version=1.1&operation=searchRetrieve&recordSchema=http%3A%2F%2Fands.org.au%2Fstandards%2Frif-cs%2FregistryObjects";
		$requestURI =  gNLA_TEST_SRU_URI."?query=rec.identifier=%22".$partyId."%22&version=1.1&operation=searchRetrieve&recordSchema=http%3A%2F%2Fands.org.au%2Fstandards%2Frif-cs%2FregistryObjects";
		$get = curl_init();
...
elseif($setIdentifiers)
{
	...
	//	$requestURI =  gNLA_SRU_URI."?query=cql.anywhere+%3D+%22".urlencode($identifiers["registry_object_key"])."%22&version=1.1&operation=searchRetrieve&recordSchema=http%3A%2F%2Fands.org.au%2Fstandards%2Frif-cs%2FregistryObjects";
		$requestURI =  gNLA_TEST_SRU_URI."?query=cql.anywhere+%3D+%22".urlencode($identifiers["registry_object_key"])."%22&version=1.1&operation=searchRetrieve&recordSchema=http%3A%2F%2Fands.org.au%2Fstandards%2Frif-cs%2FregistryObjects";
		$get = curl_init();
		curl_setopt($get, CURLOPT_URL, $requestURI);
		curl_setopt($get, CURLOPT_RETURNTRANSFER, true);
		$ch = curl_exec($get);
