...
(function ($) {
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			rpOptions: [5, 10, 15, 20, 30, 50, 100], //allowed per-page values 
			title: false,
		...
		if (p.buttons) {
			...
			for (i in p.buttons) {
				...
				if (!btn.separator) {
					...
					if(i==0){
						btnDiv.innerHTML = "<a class=\"button left smaller\">" + btn.name + "</a>";
					}else if(i==p.buttons.length-1){
						btnDiv.innerHTML = "<a class=\"button right smaller\">" + btn.name + "</a>";
					}else{
