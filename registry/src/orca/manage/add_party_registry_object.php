...
<script type="text/javascript">
	...
	tabs["#name"] = {name:"Names", cpg:"http://ands.org.au/guides/cpguide/cpgname.html"};
...
//	tabs["#coverage"] = {name:"Coverage",cpg:"http://ands.org.au/guides/cpguide/cpgcoverage.html", cError:false , cWarning:false};
	...
	function quagmire_reset() 
	{
		quagmire_init();
//Required List
		quagmire_append('REQ_PRIMARY_NAME', REQUIRED,'At least one primary name is required for the Party record.');
		quagmire_append('REQ_RELATED_OBJECT_COLLECTION', REQUIRED,'The Party must be related to at least one Collection record.');			

//Recommended List
		quagmire_append('REC_IDENTIFIER', RECOMMENDED,'At least one identifier is recommended for the Party.');
		quagmire_append('REC_LOCATION_ADDRESS', RECOMMENDED,'At least one location address is recommended for the Party.');	
		quagmire_append('REC_SUBJECT', RECOMMENDED,'At least one subject (e.g. anzsrc-for code) is recommended for the Party.');
		quagmire_append('REC_DESCRIPTION_FULL', RECOMMENDED,'At least one description (brief and/or full) is recommended for the Party. The description must be longer than 9 characters.');
		quagmire_append('REC_RELATED_OBJECT_ACTIVITY', RECOMMENDED,'It is recommended that the Party be related to at least one Activity record.'); // "Required if available"...
		quagmire_append('REC_EXISTENCEDATE', RECOMMENDED,'Existence dates are recommended for the Party.');		
	}
	quagmire_reset();
</script>
...
<table id="outer-table" summary="<?php print $action ?> Registry Object">	
	<tbody>
		...
		<td id="content-cell">
			...
			<div class="heading" style="width:95%"><h3><span id="heading_action"><?php print $action ?></span>Party</h3>
