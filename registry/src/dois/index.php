...
$doi = getPostedValue('doi');
$action = getPostedValue('action');

if( strtoupper($action) == "VIEW" )
{
	$serviceName = "getDOI";
	$parameters = "doi=".urlencode($doi);
	
	$response = doisRequest($serviceName, $parameters);
	if( $response )
	{
		if( doisGetResponseType($response) == gDOIS_RESPONSE_SUCCESS )
		{
			$doi = doisGetDOIValue($response);
			responseRedirect('view.php?doi='.urlencode($doi));
		}
	}
}

$serviceName = "listDOIS";
$response = doisRequest($serviceName, '');
...
if( $response )
{
	if( doisGetResponseType($response) == gPIDS_RESPONSE_SUCCESS )
	{
