...
/**
 * Servlet for stopping a harvest.
 * 
 * Service parameters are described below. These are key/value
 * pairings in a URL, NOT Java args.
 * 
 * <p><strong>Service: stopHarvest</strong>
 * <p><strong>Mandatory Parameters</strong>
 * <ul>
 * <li><strong>harvestid</strong> - 
 *          The harvestid of the harvest to be stopped</li></ul></p>
 * </p>
 * 
 * @author Scott Yeadon, ANU 
 */
public class ThreadStopServlet extends HttpServlet
{
    private final Logger log = Logger.getLogger(ThreadStopServlet.class);
                        ...
                        String harvestID) throws ServletException
    {
        try
        {
            ...
            {
                if (!threadManager.stop(harvest))
                {
                    log.error("Failed to stop harvest with id: " + harvestID);
                    ServletSupport.doErrorResponse(response, "Failed to stop harvest with id: " + harvestID);
                    return;
                }
                else
                {
                    log.info("Harvest " + harvestID + " stopped");
                    ServletSupport.doSuccessResponse(response, "Harvest " + harvestID + " stopped");
                }
            }
        }
